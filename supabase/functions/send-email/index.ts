    ...
    try {
      switch (messageType) {
        case "new_meeting": {
          ...
          const { error: emailErrorHost } = await resend
            .emails.send({
          ...
          if (emailErrorHost) {
            console.log("email host error", emailErrorHost);
            deleteMessage = false;
          }
          ...
          if (messageData["guest_email"]) {
            ...
            const { error: emailErrorGuest } = await resend
              .emails.send({
                from: "MyFreeHours <no-reply@myfreehours.app>",
            ...
            if (emailErrorGuest) {
              console.log("email guest error", emailErrorGuest);
              deleteMessage = false;
            }
          }
            ...
            const { error: emailErrorGuest } = await resend
              .emails.send({
                ...
                subject: "Booking with " +
                  messageData["host_name"] + " Cancelled",
                html: guestEmail,
              });
          ...
          if (messageData["guest_email"]) {
            ...
            const { error: emailErrorGuest } = await resend
              .emails.send({
                ...
                to: [messageData["guest_email"]],
                subject: "Booking with " +
                  messageData["host_name"] + " Reminder",
                html: guestEmail,
              });
