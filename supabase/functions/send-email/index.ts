...
serve(async (req: Request) => {
	...
	if (data.error) {
		return new Response(`Unable to retrieve item in email queue, sent 0 emails in total.`, {
			status: 500,
			...
			switch (messageType) {
				case 'new_meeting': {
					...
					hostEmail = await engine.parseAndRender(hostEmail, {
						...
						meeting_type: messageData['online_url'] === null ? 'in person' : 'online',
						start_time: dayjs(messageData['start_time']).format('DD/MM/YYYY, h:mm a'),
					...
					if (messageData['guest_email']) {
						...
						const { error: emailErrorGuest } = await resend.emails.send({
							...
							subject: 'Booking Confirmation with ' + messageData['host_name'],
							html: guestEmail
						});
				...
				case 'delete_meeting': {
					...
					const { error: emailErrorHost } = await resend.emails.send({
						from: 'MyFreeHours <no-reply@myfreehours.app>',
						to: [messageData['host_email']],
						subject: 'Booking with ' + messageData['guest_name'] + ' Cancelled',
						html: hostEmail
					});
					...
					if (messageData['guest_email']) {
						...
						const { error: emailErrorGuest } = await resend.emails.send({
							from: 'MyFreeHours <no-reply@myfreehours.app>',
							to: [messageData['guest_email']],
							subject: 'Booking with ' + messageData['host_name'] + ' Cancelled',
							html: guestEmail
						});
				...
				case 'pre_meeting_guest': {
					...
					if (messageData['guest_email']) {
						...
						const { error: emailErrorGuest } = await resend.emails.send({
							from: 'MyFreeHours <no-reply@myfreehours.app>',
							to: [messageData['guest_email']],
							subject: 'Booking with ' + messageData['host_name'] + ' Reminder',
							html: guestEmail
						});
				...
				case 'pre_meeting_host': {
					...
					const { error: emailErrorHost } = await resend.emails.send({
						from: 'MyFreeHours <no-reply@myfreehours.app>',
						to: [messageData['host_email']],
						subject: 'Booking with ' + messageData['guest_name'] + ' Reminder',
						html: hostEmail
					});
