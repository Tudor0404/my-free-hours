import { createEvents, DateArray, DurationObject, EventAttributes } from 'npm:ics';
...
function calculateDurationComponents(minutes: number): DurationObject {
	return {
...
function generateActiveEvent(booking: CombinedBooking): EventAttributes {
	const event: EventAttributes = {
		start: convertToDateArray(booking.start_time || ''),
		title: `${booking.type_name || 'Meeting'} w/ ${booking.guest_name} and ${booking.host_name}`,
	...
	if (booking.meeting_url) {
		event.location = booking.meeting_url;
	}
	...
	if (booking.meeting_method) {
		event.categories = [booking.meeting_method.toUpperCase(), 'MyFreeHours'];
	}

	return event;
}

export function generateDeletedEvent(booking: CombinedBooking): EventAttributes {
	const event: EventAttributes = {
		start: convertToDateArray(booking.start_time || ''),
...
export async function generateCalendar(bookings: CombinedBooking[]): Promise<string> {
	const events = bookings.map((booking) =>
		booking.is_deleted ? generateDeletedEvent(booking) : generateActiveEvent(booking)
	);
