...
export default class TimeBlock {
	...
	public static time_conjunction(
		left: TimeRange[],
		right: TimeRange[],
	): TimeRange[] {
	...
	public static time_disjunction(
		left: TimeRange[],
		right: TimeRange[],
	): TimeRange[] {
		left.push(...right);
	...
	public static time_simplification(times: TimeRange[]): TimeRange[] {
		...
		const sortedTimes = times
			...
			.toSorted((e1, e2) =>
				getAbsoluteTime(e1.time) - getAbsoluteTime(e2.time)
			);
		...
		for (let i = 0; i < sortedTimes.length; i++) {
			...
			if (time.type == "end") {
				if (passedOver.length == 1) {
					reducedTimes.push(
						createRange(times[curStart].start, times[passedOver[0]].end),
					);
					curStart = -1;
					passedOver = [];
				} else {
	...
	public static decode_json(obj: Record<string, any>): TimeBlock {
		...
		if (!(obj.hasOwnProperty("operator") && obj["operator"] == "BETWEEN")) {
			throw new Error(
				"A time block should have an operator with a value of BETWEEN",
			);
		}
		...
		if (obj.hasOwnProperty("values") && obj["values"] instanceof Array) {
			if (obj["values"].length == 2) {
				...
				throw new Error(
					"Only 2 values must be in the values field in a time block",
				);
			}
		} else {
