...
export default class ScheduleBlock {
	...
	public evaluate(date: Dayjs): TimeRange[] {
		if (this.cachedSchedule === null) {
			...
			this.cachedSchedule = Schedule.decode_json(JSON.parse((s.schedule as string).toString()));
		}
	...
	public clone() {
		return new ScheduleBlock(this.id);
	}
	...
	public static decode_json(obj: Record<string, any>): ScheduleBlock {
		...
		if (!(obj.hasOwnProperty('operator') && obj['operator'] == 'IN')) {
			throw new Error('A schedule block should have an operator with a value of IN');
		}
		...
		if (obj.hasOwnProperty('values') && obj['values'] instanceof Array) {
			if (obj['values'].length == 1) {
				...
				throw new Error('Only 1 value must be in the values field on a schedule block');
			}
		} else {
