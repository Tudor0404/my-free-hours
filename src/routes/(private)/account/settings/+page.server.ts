...
export const load = async ({ locals: { supabase }, depends }) => {
	const { data: userData } = await supabase.from('user').select('*').limit(1).single();

	const { data: blacklistDaysData } = await supabase.from('blacklisted_days').select('date');

	const { data: icsLink } = await supabase.from('booking_page').select('calendar_id').single();
	...
	const blacklistForm = await superValidate(
		blacklistDaysData ? { days: blacklistDaysData.map((item) => item.date) } : null,
		zod(blacklistDays)
	);
...
export const actions = {
	...
	updateBlacklistDays: async ({ request, locals: { supabase } }) => {
		...
		const toRemove = existingDays.filter((item) => !form.data.days.includes(item));
